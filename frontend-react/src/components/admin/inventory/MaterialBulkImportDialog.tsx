...
type MaterialBulkImportDialogProps = {
  open: boolean;
...
export default function MaterialBulkImportDialog({
  open,
  onOpenChange,
  onCompleted,
}: MaterialBulkImportDialogProps) {
  ...
  const handleDownloadTemplate = () => {
    XLSX.writeFile(buildTemplateWorkbook(), "template-bulk-materials.xlsx");
  };
  ...
  const handleFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const categoryValues = new Set(
        MATERIAL_CATEGORY_OPTIONS.map((item) => item.value),
      );
  ...
  const handleSubmitBulk = async () => {
    ...
    const selectedRows = previewRows
      ...
      .map((row) => ({
        ...row,
        roomId: rowRoomSelections[row.index] || undefined,
      }));
  ...
  return (
    <BulkImportDialogShell
      ...
      onReset={resetState}
      title="Bulk Import Bahan"
      description={
        <>
          Upload file Excel untuk membuat banyak bahan sekaligus. Pilih baris
          yang akan diimport, lalu assign ruangan per baris (opsional).
        </>
      }
            ...
            <table className="w-full min-w-[1100px] text-left text-xs">
              <thead className="sticky top-0 z-10 bg-muted/40">
                <tr>
                  ...
                  <th className="w-[260px] px-2 py-2 font-medium">Ruangan</th>
      ...
      {skippedRows.length ? (
        <div className="space-y-2 rounded-md border border-amber-200 bg-amber-50/60 p-3">
          ...
          <div className="max-h-40 space-y-1 overflow-y-auto text-xs text-amber-900">
            {skippedRows.map((row) => (
              <p key={`skipped-material-${row.index}`}>
